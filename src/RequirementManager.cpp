	...
	const MojObject& value)
{
	...
	LOG_AM_DEBUG("Looking up specific requirement manager to instantiate [Requirement %s]",
		name.c_str());

	RequirementMap::iterator found = m_requirements.find(name);
	if (found == m_requirements.end()) {
		LOG_AM_ERROR(MSGID_REQ_MANAGER_NOT_FOUND,1,
...
{
	LOG_AM_TRACE("Entering function %s", __FUNCTION__);
	LOG_AM_DEBUG("Registering [Manager %s] for [Requirement %s]",
...
{
	LOG_AM_TRACE("Entering function %s", __FUNCTION__);
	LOG_AM_DEBUG("Unregistering [Manager %s] from [Requirement %s]",
		manager->GetName().c_str(), name.c_str());

	RequirementMap::iterator found = m_requirements.find(name);
	if (found == m_requirements.end()) {
		LOG_AM_DEBUG("No manager found for [Requirement %s], attempting to unregister [Manager %s]",
			name.c_str(),manager->GetName().c_str());
	} else if (found->second != manager) {
		LOG_AM_DEBUG("[Manager %s] not currently registered for [Requirement %s], it is registered to [Manager %s]",
